...
const updateFavoriteStore = (items, changedStoreId, is_favoritestore) => {
  return items.map(item => {
    if (item.store_id._id === changedStoreId) {
      return {
        ...item,
        store_id: {
          ...item.store_id,
  ...
  const changeFav = async st => {
    ...
    if (st) {
      try {
        ...
        const resGoods = updateFavoriteStore(
          goods,
          shop._id,
          arr.is_favoritestore,
        );

        dispatch({type: 'SET_GOODS', payload: resGoods});
      } catch (e) {
        console.log(e, 3);
      }
    } else {
        ...
        {active === 'Все товары' && (
          <FlatList
            data={goods.filter(item =>
              goodsData.map(item => item._id).includes(item._id),
            )}
